...
import random
import socket
import threading  # FIX: added for non-blocking trigger reset
import time
...
# FIX: send_trigger() is now non-blocking.
# The original version called time.sleep(TRIGGER_PULSE_DUR) on the main thread,
# which blocked PsychoPy's event loop (Pyglet) for 50 ms every trial, causing
# Unicorn Recorder to lose sync and PsychoPy's behavioral CSV to stop recording.
# Now the 50 ms wait and the reset are handled by a daemon background thread,
# so the main thread is never stalled.
def _reset_trigger_after_delay():
    """Background thread target: waits 50 ms then sends byte 0 (reset)."""
    time.sleep(TRIGGER_PULSE_DUR)
    # FIX: was b"0" (byte value 48 = ASCII '0'); must be bytes([0]) (integer 0)
...
def send_trigger(code):
    ...
    Sends the trigger code as a raw integer byte (FIX: was ASCII text encoding),
    then schedules the 50 ms reset on a daemon background thread.

    Old (broken):
        str(code).encode()  ->  b"1" for code=1 (byte value 49, not 1)
        time.sleep(0.05)    ->  blocks main thread / PsychoPy event loop
        b"0"                ->  byte value 48, not 0

    New (correct):
        bytes([code])       ->  byte value == code (e.g., 1 for oddball_standard)
        thread handles sleep + reset, main thread never blocks
        bytes([0])          ->  byte value 0 = proper reset
    """
    # FIX: use raw integer byte, not ASCII text
    sock.sendto(bytes([code]), endpoint)
    # FIX: reset on background thread — main thread does not sleep
...
def save_trial(writer, row_dict, fieldnames, file_obj=None):
    ...
    FIX: accepts optional file_obj parameter. When provided, flushes the file
    buffer to disk immediately after each row write. This prevents data loss
    if the experiment crashes or is interrupted — without flush(), Python's I/O
    buffer may hold many rows in memory and never write them if the process
...
def run_stimulus_for_duration(win, image_stim, duration, response_key=None, marker_code=None):
    ...
    Opțional:
    - trimite markerul sincronizat cu flip-ul ecranului
    - colectează răspunsul participantului

    FIX: trigger is now scheduled via win.callOnFlip() BEFORE the first flip,
    not called after win.flip() returns. This ensures the UDP packet goes out
    at the exact moment the image appears on screen (hardware flip callback),
    which is the correct approach for EEG time-locking.

    The old approach called send_trigger() after win.flip() returned, which
    added ~1 frame of latency (~16 ms at 60 Hz) and — because send_trigger()
    contained time.sleep(0.05) — blocked the main thread for 50 ms on every
    trial, disrupting PsychoPy's Pyglet event loop and causing Unicorn
    Recorder to lose its recording state.
    """
    event.clearEvents(eventType="keyboard")
    clock = core.Clock()

    # FIX: schedule the trigger to fire at the hardware flip moment, not after.
    # win.callOnFlip() calls the function at the very next buffer swap.
    # send_trigger() is now non-blocking (see its definition above).
    ...
    image_stim.draw()
    win.flip()
    # imaginea apare pe ecran; trigger fires here via callOnFlip callback
...
def run_oddball_practice(win, writer, fieldnames, participant_code, practice_trials, image_cache, file_obj=None):
    # file_obj: FIX — passed through to save_trial() for immediate flush after each row
    draw_text_and_wait(
        win,
        "Exersare\n\n"
...
def main():
    ...
    with open(outfile, "w", newline="", encoding="utf-8") as f:
        ...
        writer.writeheader()
        f.flush()  # FIX: flush the header to disk immediately
        ...
        run_oddball_block(
            ...
            practice_trials=practice_trials,
            image_cache=image_cache,
            file_obj=f  # FIX: pass file handle so save_trial() can flush
        )
